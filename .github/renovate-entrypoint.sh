...
# The shared home-operations preset attaches helm-docs and helm-schema as
# post-upgrade tasks. Renovate's container carries neither, and `mise run
# generate-check` fails on a chart README that was not regenerated, so a
# Renovate pull request that skipped them could never pass this repository's
# own gate. Versions are pinned and Renovate updates them here like any other
# dependency.
