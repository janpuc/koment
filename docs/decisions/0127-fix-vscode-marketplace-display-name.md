...
Date: 2026-08-07
Status: Accepted
Supersedes: [0126](0126-fix-vscode-marketplace-extension-name.md)
...
- `DESIGN.md` row "Editor distribution" — note that the marketplace
  `displayName` is `koment-dev`.
- `docs/development.md` — the local recommendation of the `koment.koment`
  extension becomes `koment.koment-dev` (the row was already on the old id).

No badge or external link in the README references `displayName`, so no
README change is needed. The README badge still points at
`open-vsx.org/extension/koment/koment-dev` and
`marketplace.visualstudio.com/items?itemName=koment.koment-dev`, which are
the `(publisher, name)` URLs and remain correct.
...
- ADR 0126 is now incorrect on one of its two decisions. It is marked
