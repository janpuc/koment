...
func TestDraftAnnotationAnchorsAndAttributesTheExactSnapshot(t *testing.T) {
	...
	record, err := DraftAnnotation(snapshot, AddInput{
		File: "main.go", Excerpt: "serve()", Kind: store.KindWhy, Body: "The call owns the process lifetime.",
		Author: store.Author{Name: "Remote Agent", Kind: store.AuthorAgent, Source: store.FromScopedAgent, Verified: "bearer-sha256"},
	})
	if err != nil {
		t.Fatal(err)
	}
	if record.Anchor.LastSeenLine != 3 || record.Git.Commit != snapshot.Commit || record.Git.Path != "main.go" {
...
func TestDraftAnnotationRefusesMissingOrAmbiguousSource(t *testing.T) {
	...
	for _, input := range []AddInput{
		{File: "absent.go", Kind: store.KindWhy, Body: "Missing", Author: author},
		{File: "main.go", Excerpt: "serve()", Kind: store.KindWhy, Body: "Ambiguous", Author: author},
	} {
